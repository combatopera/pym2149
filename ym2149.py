...
class YM2149(Mixer):
  ...
  def __init__(self):
    ...
    Mixer.__init__(*[self] + [Dac(BinMix(ToneOsc(self.toneperiods[i]), noise, self.toneflags[i], self.noiseflags[i]), self.levelmodes[i], self.fixedlevels[i], env, self.channels) for i in xrange(self.channels)])
